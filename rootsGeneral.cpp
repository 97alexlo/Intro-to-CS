...
int main() {
    ...
    for (int i = 0; i < numRows; i++) {
        ...
        for( int j = 1; j <= maxIterations; j++) {
            nextEst = newtonsMethod(prevEst, fx(prevEst, size), fxPrime(prevEst, size));
            if (j < 3) {
...
double fxPrime(double x, int size) {
    double a = 0.0;
